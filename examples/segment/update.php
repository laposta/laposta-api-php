...
if (!$segmentId) {
    echo "Error: LP_EX_SEGMENT_ID environment variable is not set. "
        . "This is required to identify the segment to update.\n";
    exit(1);
}
...
$updateData = [
    'name' => 'Subscription date - updated - ' . date('Y-m-d H:i:s'),
];
try {
    $result = $segmentApi->update($listId, $segmentId, $updateData);
    echo "Segment with ID '$segmentId' in list '$listId' updated successfully:\n";
    print_r($result);
