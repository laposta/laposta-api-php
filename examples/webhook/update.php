...
if (!$webhookId) {
    echo "Error: LP_EX_WEBHOOK_ID environment variable is not set. "
        . "This is required to identify the webhook to update.\n";
    exit(1);
}
...
// Update webhook
$updateData = [
    'event_types' => ['subscribed', 'unsubscribed'],
    'description' => 'My updated example webhook - ' . date('Y-m-d H:i:s'),
];
try {
    $result = $webhookApi->update($listId, $webhookId, $updateData);
    echo "Webhook with ID '$webhookId' for list_id '$listId' updated successfully:\n";
    print_r($result);
